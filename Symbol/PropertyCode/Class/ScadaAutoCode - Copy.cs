    ...
    public static class ScadaAutoCode
    {
      ...
      //  public static IEasyDriverConnector connector;
        ...
        private static void OnTagColorValueChanged(string key)
        {
            ...
            string val = ExeExpression((ListCode[key] as ColorSetting).Expression).Split('.')[0]; // đọc giá trị sau đó thế vào string rồi so sánh trả ra string
            if ((ListCode[key] as ColorSetting).ColorObjects.Count > 0)
            {
                ColorSetting d = ListCode[key] as ColorSetting;
                foreach (ColorObject obj in (ListCode[key] as ColorSetting).ColorObjects)
                {
                    ...
                    else
                    {
                        if (val == obj.Value)
                        {
                 //           SetProperty(control, key, Color.Red);
